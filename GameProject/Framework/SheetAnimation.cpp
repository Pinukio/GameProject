...
{
	sheet = Scene::GetCurrentScene().GetResourceManager().LoadBitmapFromFile(sheetPath,0,0);
	if (sheet)
	{
		this->length = length;
		if (width != 0)
...
void SheetAnimation::SetSourceRect(D2D1_RECT_F& outRect)
{
	outRect = sourceRects[currentFrame];
}
...
Vector2 SheetAnimation::GetAnimationSize()
{
