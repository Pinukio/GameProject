	...
	col(*transform, Vector2(42.0f, 128.0f))
{
	...
	ListAnimation* lanim = new ListAnimation(3, 1.0f);		//애니메이션 생성, 해당 애니메이션의 state는 0, 초당 1회 그림
	lanim->PushBackSprite(L"a.png");				//이미지 등록
	lanim->PushBackSprite(L"b.png");
	SheetAnimation* sanim = new SheetAnimation(4, 16.0f,L"sheet.png",128,128,16);	//애니메이션 생성, 해당 애니메이션의 state는 1, 초당 16회 그림
...
void Player::Update()
{
	...
	{
		if (animRenderer->currentState==4)
		{
			animRenderer->ChangeAnimation(3);	//ChangeAnim(state)를 통해 애니메이션 전환 가능
		}
		else
		{
			animRenderer->ChangeAnimation(4);
		}
	}
